...
/*   Updated: 2024/04/18 17:41:01 by jmigoya-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/miniRT.h"

void	convert_camera(t_mini_rt_data *data, char *coords_vp, char *v_3d_norm,
		char *fov)
{
	...
	if (!is_float(fov))
		handle_error(ERR_PARSE_DATA, "Camera: fov's format is not correct",
			data);
	data->cam->fov = ft_atof(fov);
	free(fov);
	if (!data->err_code && !is_float_in_range(data->cam->fov, 0, 180))
		handle_error(ERR_PARSE_DATA, "Camera: fov is out of range [0, 180]",
			data);
}
